 ...
 * @return true if everything went fine
 */
bool UAVObjectGeneratorGCS::generateEnumDefinitions(FieldInfo* field, QString& enums, QString& q_enums)
{

    bool res = true;

    QString buffer = QString("");
    QString buffer_qml = QString("");
...
//            buffer_qml.append(QString("    Q_ENUMS(%1Options)\r\n").arg(info->fields[n]->name));
...
//            buffer_qml.append(QString("    Q_ENUMS(%1Elem)\n").arg(info->fields[n]->name));
        }
    ...
    q_enums.prepend(buffer_qml);

    // process subfields
    foreach(FieldInfo* childField, field->childrenFields) {
        if(childField->type == FIELDTYPE_STRUCT)
            res = res && generateEnumDefinitions(childField, enums, q_enums);
    }
