...
PathPlannerStates direct_path_planner(uint16_t numberOfWaypoints)
{
	...
	for(int wptIdx=0; wptIdx<numberOfWaypoints; wptIdx++) {
		...
		if ( !path_is_circle ) {
			...
			uint8_t ret;
			ret = addCircleToSwitchingLoci(waypoint.Position, finalVelocity, curvature, numberOfOrbits, wptIdx+offset);
			offset += ret;
		}
	}
...
static uint8_t addNonCircleToSwitchingLoci(float position[3], float finalVelocity, float curvature, uint16_t index)
{
	...
	pathSegmentDescriptor.Timeout = 51; // TODO: Calculate time
 ...
 * @return
 */
static uint8_t addCircleToSwitchingLoci(float circle_center[3], float finalVelocity, float curvature, float numberOfOrbits, uint16_t index)
{
	...
	if (d > pathManagerSettings.HalfPlaneAdvanceTiming*pathSegmentDescriptor.FinalVelocity) {
		...
		pathSegmentDescriptor.NumberOfOrbits = 0;
		pathSegmentDescriptor.ArcRank = PATHSEGMENTDESCRIPTOR_ARCRANK_MINOR;
		pathSegmentDescriptor.Timeout = 52; // TODO: Calculate timeout
		PathSegmentDescriptorInstSet(index, &pathSegmentDescriptor);
		...
		pathSegmentDescriptor.ArcRank = PATHSEGMENTDESCRIPTOR_ARCRANK_MINOR;
		pathSegmentDescriptor.Timeout = 53; // TODO: Calculate timeout
		...
		pathSegmentDescriptor.NumberOfOrbits = 0;
		pathSegmentDescriptor.ArcRank = PATHSEGMENTDESCRIPTOR_ARCRANK_MINOR;
		pathSegmentDescriptor.Timeout = 54; // TODO: Calculate timeout
		PathSegmentDescriptorInstSet(index, &pathSegmentDescriptor);
		...
		pathSegmentDescriptor.NumberOfOrbits = numberOfOrbits;
		pathSegmentDescriptor.ArcRank = PATHSEGMENTDESCRIPTOR_ARCRANK_MINOR;
		pathSegmentDescriptor.Timeout = 55; // TODO: Calculate timeout
		PathSegmentDescriptorInstSet(index+1, &pathSegmentDescriptor);
	}
