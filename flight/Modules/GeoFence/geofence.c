...
static void geofenceTask(void *parameters)
{
	...
	while(1) {
		...
		for (typeof(num_faces) i=0; i<num_faces; i++) {
			...
			float vertexA[3];
			LLA2NED(vertexA_LLH, geofenceVerticesData.Altitude, vertexA);
			...
			float vertexB[3];
			LLA2NED(vertexB_LLH, geofenceVerticesData.Altitude, vertexB);
			...
			float vertexC[3];
			LLA2NED(vertexC_LLH, geofenceVerticesData.Altitude, vertexC);
